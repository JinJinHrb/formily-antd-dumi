...
import { useFormLayout, FormLayoutShallowContext, FormLayoutDeepContext } from '../form-layout'
...
const useFormItemLayout = (props: IFormItemProps) => {
	...
	let labelAlign = props.labelAlign ?? formLayout.labelAlign ?? 'right'
	if (formLayout.layout === 'vertical' || formLayout.layout === 'two-columns') {
		labelAlign = props.labelAlign ?? formLayout.labelAlign ?? 'left'
	}

	const layout = props.layout ?? formLayout.layout ?? 'horizontal'

	return {
		...props,
		layout,
		colon: props.colon ?? formLayout.colon ?? (layout === 'vertical' ? false : true),
		labelAlign,
		labelWrap: props.labelWrap ?? formLayout.labelWrap,
		labelWidth: props.labelWidth ?? formLayout.labelWidth,
		wrapperWidth: props.wrapperWidth ?? formLayout.wrapperWidth,
		labelCol: props.labelCol ?? formLayout.labelCol,
		wrapperCol: props.wrapperCol ?? formLayout.wrapperCol,
		wrapperAlign: props.wrapperAlign ?? formLayout.wrapperAlign,
		wrapperWrap: props.wrapperWrap ?? formLayout.wrapperWrap,
		fullness: props.fullness ?? formLayout.fullness,
		size: props.size ?? formLayout.size,
		inset: props.inset ?? formLayout.inset,
		asterisk: props.asterisk,
		bordered: props.bordered ?? formLayout.bordered,
		feedbackIcon: props.feedbackIcon,
		feedbackLayout: props.feedbackLayout ?? formLayout.feedbackLayout ?? 'loose',
		tooltipLayout: props.tooltipLayout ?? formLayout.tooltipLayout ?? 'icon',
		tooltipIcon: props.tooltipIcon ?? formLayout.tooltipIcon ?? <QuestionOutlined />,
	}
}
	...
	const renderLabel = () => {
		...
		return (
			<div
				...
				{renderLabelText()}
				{renderTooltipIcon()}
				{label !== ' ' && colon && <span className={`${prefixCls}-colon`}>{colon ? ':' : ''}</span>}
			</div>
		)
	}
	return (
			...
			<div
				...
				<div className={cls(`${prefixCls}-control-content`)}>
					...
					<div
						...
						<FormLayoutDeepContext.Provider value={{}}>
